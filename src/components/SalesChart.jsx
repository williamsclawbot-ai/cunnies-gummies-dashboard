...
function SalesChart({ data }) {
  const chartData = useMemo(() => {
    // LOCKED: Always show monthly trend (Oct 2025 - Feb 2026)
    // Ignore selectedMonth - this chart always displays all-time monthly data
    const filtered = data
      .filter(m => m.orders > 0)
      .map(m => ({
  ...
  const chartTitle = 'Monthly Sales Trend (All Time)';
